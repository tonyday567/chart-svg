...
#+RESULTS:
: [14 of 14] Compiling Chart.Examples   ( src/Chart/Examples.hs, interpreted ) [Source file changed]
...
- [X] text text is not showing (dark mode?)
- [ ] bar text numbers not exactly mid
- [ ] glyphs - last glyph cut off, entire chart should be thin.
- [ ] path.
  - [ ] text on chart dark in dark mode.
  - [ ] chart data area not accounting for chart text


#+begin_src haskell
writeChartOptions "other/bardebug.svg" $ debugExample barExample
#+end_src

#+RESULTS:
: <interactive>:53:1: error:
:     Variable not in scope: writeChartOption :: t0 -> ChartOptions -> b
:     Suggested fix:
:       Perhaps use ‘writeChartOptions’ (imported from Chart)
