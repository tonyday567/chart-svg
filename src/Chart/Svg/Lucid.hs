...
styleBoxGlyph s = move p' $ sw $ case sh of
  ...
  VLineGlyph a -> NH.scale (Point (a * sz) sz) unitRect
  HLineGlyph a -> NH.scale (Point sz (a * sz)) unitRect
...
svgShape (EllipseGlyph x') s p@(Point x y) =
  ellipse_
  [ cx_ (show x)
  , cy_ (show $ -y)
  , rx_ (show $ 0.5 * s)
...
svgShape (VLineGlyph x') s (Point x y) =
  polyline_ [points_ (show x <> "," <> show (- (y - (x'*s) / 2)) <> "\n" <> show x <> "," <> show (- (y + (x'*s) / 2)) )]
svgShape (HLineGlyph x') s (Point x y) =
  polyline_ [points_ (show (x - (x'*s) / 2) <> "," <> show (-y) <> "\n" <> show (x + (x'*s) / 2) <> "," <> show (-y))]
{-
svgShape (PathGlyph pcs) _ _ =
  path_ pcs
-}
...
attsPixel o =
  ...
  , fill_ ("url(#" <> show (o ^. #pixelTextureId) <> ")")
  ]
