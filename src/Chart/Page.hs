...
repTitle txt cfg = do
  ...
  ts <- repTextStyle (cfg^. field @"style")
  tp <- repPlace (cfg ^. field @"place")
  ta <- repAnchor (cfg ^. field @"align")
...
repTextStyle s = do
  ...
  tc <- color_ "color" (s ^. field @"color")
  to' <- stepsliderR_ "opacity" 0 1 0.1 (s ^. field @"opacity")
  ta <- repAnchor (s ^. field @"alignH")
  ...
  trc <- maybeRep "rotation" "trc" (maybe False  (const True) (s ^. field @"rotation"))
...
repCanvasConfig cfg = do
  canvasc <- color_ "Canvas Color" (cfg ^. field @"color")
...
repRectStyle s = do
  ...
  bc <- color_ "border color" (s ^. field @"borderColor")
  bo <- stepsliderR_ "border opacity" 0 1 0.1 (s ^. field @"borderOpacity")
  c <- color_ "color" (s ^. field @"color")
...
repBar cfg = do
  p <- repPlace (cfg ^. field @"place")
  r <- repRectStyle (cfg ^. field @"rstyle")
...
repAxisConfig cfg = do
  b <-
    ...
    (repBar (maybe defaultBar identity (cfg ^. field @"abar")))
